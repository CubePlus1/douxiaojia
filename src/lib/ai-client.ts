...
## instructions
- 必须是一段 **markdown 字符串**（不是数组！不要用 JSON array）
- 内部可以用短横线列表或编号列表，但整个 instructions 字段只能是 1 个 string
- 写出详细、具体、可执行的指令，融合视频中的实际知识点和方法论
- 告诉 AI 在什么场景下应该如何帮助用户
- 不要写空泛的指令（如「提供帮助」），要写具体行为（如「当用户询问 AI 绘画工具时，推荐 Midjourney、Stable Diffusion 等，并给出参数配置建议」）

示例（正确格式）：
"instructions": "你是 XXX 知识助手。当用户询问 Y 时：\\n- 先做 A\\n- 再做 B\\n- 必要时引用视频中的 Z"

错误格式（不要这样）：
"instructions": ["做 A", "做 B", "做 C"]   ← 这是数组，会校验失败
